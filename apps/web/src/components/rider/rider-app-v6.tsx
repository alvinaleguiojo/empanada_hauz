...
export function RiderAppV6() {
  ...
  const hydrate = useCallback(async () => {
    ...
    setProfile(nextProfile); setJobs(nextJobs);
...
function NavigationScreen({ job, riderLocation, heading, navigating, onBack, onAdvance, busy }: { job: Job; riderLocation: Coordinate | null; heading: number; navigating: boolean; onBack: () => void; onAdvance: (job: Job) => void; busy: boolean }) {
  ...
  useEffect(() => {
    ...
    fetch(url).then(response => response.ok ? response.json() : Promise.reject(new Error(`Routing service returned ${response.status}`))).then(data => {
      ...
      setRoute(nextRoute);
    }).catch(err => { if (!cancelled) setRouteError(err instanceof Error ? err.message : "Unable to calculate route"); });
  ...
  useEffect(() => {
    ...
    if (!map || !L) return;
    layers.current.forEach(layer => { try { map.removeLayer(layer); } catch {} }); layers.current = [];
    if (riderLocation) {
      ...
      layers.current.push(L.marker([riderLocation.latitude, riderLocation.longitude], { icon }).addTo(map));
    }
    if (destination) {
      ...
      layers.current.push(L.marker([destination.latitude, destination.longitude], { icon }).addTo(map));
    }
    const coordinates = route?.geometry?.coordinates;
    if (Array.isArray(coordinates) && coordinates.length > 1) {
      const points: [number, number][] = coordinates.map(([lng, lat]) => [lat, lng]);
      layers.current.push(L.polyline(points, { color: "#F4581D", weight: 8, opacity: 0.9, lineCap: "round", lineJoin: "round" }).addTo(map));
