...
const monthFormatter = new Intl.DateTimeFormat("en-US", { month: "long", year: "numeric", timeZone: "Asia/Manila" });
const dayFormatter = new Intl.DateTimeFormat("en-US", { weekday: "short", timeZone: "Asia/Manila" });
...
function calendarDays(year: number, month: number) {
  const first = startOfCalendarMonth(year, month);
...
export function OrdersCalendar({ orders }: { orders: CalendarOrder[] }) {
  ...
  const selectedOrder = upcomingOrders.find((order) => order.id === selectedId) ?? upcomingOrders[0] ?? null;
  const currentMonthKey = monthStartKey(viewDate.getFullYear(), viewDate.getMonth());
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="h-10 rounded-lg border border-white/[0.09] bg-[#101827] px-3 text-sm text-foreground outline-none focus:border-accent/60">
              ...
              {Array.from(new Set(upcomingOrders.map((order) => order.status).filter(Boolean))).map((status) => (
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_330px]">
        <Card className="overflow-hidden p-0">
          <div className="grid grid-cols-7 border-b border-white/[0.08] bg-white/[0.025]">
            {Array.from({ length: 7 }, (_, index) => {
              const date = new Date(2026, 7, 16 + index);
              return <div key={index} className="px-2 py-2 text-center text-[11px] font-semibold uppercase tracking-[0.14em] text-foreground/42">{dayFormatter.format(date)}</div>;
            })}
          </div>
          ...
          <div className="grid grid-cols-7 auto-rows-[minmax(112px,1fr)]">
            {days.map((day) => {
              ...
              const isCurrentMonth = key.slice(0, 7) === currentMonthKey.slice(0, 7);
              ...
              return (
                <div key={key} className={cn("min-w-0 border-b border-r border-white/[0.065] p-1.5 sm:p-2", !isCurrentMonth && "bg-white/[0.012]", isToday && "bg-accent/[0.035]")}>
                  <div className="mb-1 flex h-6 items-center justify-between">
                    <span className={cn("grid h-6 min-w-6 place-items-center rounded-full px-1 text-xs", isToday ? "bg-accent text-white font-semibold" : isCurrentMonth ? "text-foreground/70" : "text-foreground/25")}>{day.getDate()}</span>
        ...
        <Card className="h-fit xl:sticky xl:top-4">
          <div className="flex items-center gap-2 border-b border-white/[0.08] pb-4">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.16em] text-foreground/40">Upcoming orders</p>
              <p className="text-lg font-semibold">{upcomingOrders.length}</p>
            </div>
          </div>
