      ...
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-5">
        ...
        <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
          <div className="rounded-lg border border-line/80 bg-panel p-4 sm:p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <span
                className={cn(
                  "inline-flex w-fit rounded-md px-3 py-1.5 text-sm font-semibold capitalize",
                    ...
                    <div key={step.key} className="grid grid-cols-[32px_minmax(0,1fr)] gap-3">
                      <div
                        className={cn(
                          ...
                          complete ? "border-accent bg-accent text-white" : "border-line bg-black/10 text-foreground/35"
                        )}
                      >
          ...
          <aside className="grid gap-4">
            <InfoPanel
              icon={Clock3}
              label="Schedule"
              value={order.preferredSchedule ? formatDate(order.preferredSchedule) : "Not scheduled yet"}
            />
            <InfoPanel
              icon={MapPin}
        ...
        <section className="grid gap-4 lg:grid-cols-2">
                  ...
                  <div key={`${item.name}-${item.quantity}`} className="flex items-start justify-between gap-3 rounded-lg border border-line/70 bg-black/[0.06] px-3.5 py-3">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-sm text-foreground/50">
                        {item.quantity} pcs{item.price !== undefined ? ` x Php ${formatPeso(item.price)}` : ""}
                      </p>
                    </div>
          ...
          <div className="rounded-lg border border-line/80 bg-panel p-4 sm:p-5">
            ...
            {note ? (
              <p className="mt-4 whitespace-pre-wrap rounded-lg border border-line/70 bg-black/[0.06] px-3.5 py-3 text-sm leading-6 text-foreground/75">
                {note}
              </p>
            ) : (
...
function InfoPanel({ icon: Icon, label, value }: { icon: typeof Clock3; label: string; value: string }) {
  return (
    <div className="rounded-lg border border-line/80 bg-panel p-4">
      <div className="flex items-start gap-3">
        <Icon size={18} className="mt-0.5 shrink-0 text-accent" />
        <div className="min-w-0">
          <p className="text-xs uppercase tracking-[0.16em] text-foreground/35">{label}</p>
...
function MaximTrackingPanel({ order }: { order: TrackingOrder }) {
  const steps = [
    {
      label: "Preparing for Booking",
      complete: ["ready_for_booking", "booked", "completed"].includes(order.status),
      description: "Your order is being prepared for Maxim booking."
    },
    {
      label: "Maxim Rider Booked",
      complete: order.status === "booked" || order.status === "completed" || order.delivery?.status === "booked",
      description: "A Maxim booking has been arranged by the store."
    },
    {
      label: "Delivered",
      complete: order.status === "completed" || order.delivery?.status === "completed",
      description: "The order has been marked completed."
    }
  ];
  ...
  return (
    <section className="rounded-lg border border-line/80 bg-panel p-4 sm:p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <span className="w-fit rounded-md bg-accent/15 px-3 py-1.5 text-sm font-semibold capitalize text-accent">
          {order.delivery?.status ?? (order.status === "booked" ? "booked" : "pending")}
        </span>
      </div>

      <div className="mt-5 grid gap-3 md:grid-cols-3">
        {steps.map((step) => (
          <div key={step.label} className={cn("rounded-lg border px-4 py-4", step.complete ? "border-accent/45 bg-accent/[0.08]" : "border-line/70 bg-black/[0.05]")}>
            <div className={cn("flex h-9 w-9 items-center justify-center rounded-full", step.complete ? "bg-accent text-white" : "bg-black/15 text-foreground/38")}>
              {step.complete ? <CheckCircle2 size={18} /> : <PackageCheck size={18} />}
            </div>
            <p className="mt-3 font-semibold">{step.label}</p>
            <p className="mt-1 text-sm leading-6 text-foreground/52">{step.description}</p>
          </div>
        ))}
      </div>
      ...
      {order.delivery?.bookingNotes ? (
        <div className="mt-4 rounded-lg border border-line/70 bg-black/[0.06] px-3.5 py-3">
          <p className="text-xs uppercase tracking-[0.16em] text-foreground/35">Booking Notes</p>
          <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-foreground/72">{order.delivery.bookingNotes}</p>
        </div>
      ) : null}
    </section>
  );
...
function getPublicNote(order: TrackingOrder) {
  const noteBodies = [
    order.notes,
    ...(order.orderNotes ?? []).map((note) => note.body)
  ]
...
function formatDate(value?: string | null) {
  return value ? scheduleFormatter.format(new Date(value)) : "Not available";
}
