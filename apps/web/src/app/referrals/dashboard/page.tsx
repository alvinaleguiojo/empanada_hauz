...
export default async function ReferralPartnerDashboardPage() {
  ...
  if (!token) {
    redirect("/referrals/login");
  }

  const status = await apiFetch<{ approved: boolean }>("/referrals/partners/status", undefined, token).catch(() => ({ approved: false }));
  if (!status.approved) {
    redirect("/referrals/pending" as Route);
  }

  const initialData = token
    ? await apiFetch<ReferralsOverview>("/referrals/partners/me", undefined, token).catch(() => emptyReferrals)
    : emptyReferrals;
