    ...
    <main className="mx-auto w-full max-w-6xl px-5 py-8 lg:px-8">
      ...
      {role && visibleSections.length > 0 ? (
        ...
        <Card className="p-8 text-center">
          <p className="font-semibold">No settings permissions</p>
          <p className="mt-1 text-sm text-foreground/50">Your current role does not have access to any settings management area.</p>
        </Card>
      ) : (
        <Card className="p-8 text-sm text-foreground/50">Loading access permissions…</Card>
      )}
      ...
      <Card className="mt-6 p-5">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-foreground/55">Settings cards are filtered from the signed-in role, while the API independently enforces the same permission matrix. Admins can manage users and roles; other roles only see settings they are authorized to use.</p>
          </div>
        </div>
      </Card>
    </main>
