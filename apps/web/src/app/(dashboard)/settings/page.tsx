...
import { Bot, KeyRound, Package, Save, ShieldCheck, Users } from "lucide-react";
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (!role || !hasPermission(role, "ai-instructions.manage")) {
    ...
    void apiFetch<AiModelSettings>("/ai-actions/model")
      .then((settings) => {
        setAiModel(settings);
        const matching = aiModelOptions.find((option) => option.provider === settings.provider && option.model === settings.model);
        setAiModelValue(matching?.value ?? `${settings.provider}:${settings.model}`);
      })
  ...
  async function saveAiModel() {
    const option = aiModelOptions.find((item) => item.value === aiModelValue);
    if (!option) return;
    setAiModelSaving(true);
    setAiModelNotice(null);
    setAiModelError(null);
    try {
      const saved = await apiFetch<AiModelSettings>("/ai-actions/model", {
        method: "PATCH",
        body: JSON.stringify({ provider: option.provider, model: option.model })
      });
      setAiModel(saved);
      setAiModelNotice(`AI model switched to ${option.label}.`);
    } catch (error) {
      setAiModelError(error instanceof Error ? error.message : "Unable to save AI model settings.");
    } finally {
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-5 py-8 lg:px-8">
      ...
      {role && hasPermission(role, "ai-instructions.manage") ? (
