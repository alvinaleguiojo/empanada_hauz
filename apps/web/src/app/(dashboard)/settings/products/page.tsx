...
import { CheckCircle2, ImagePlus, Pencil, Plus, Sparkles, Trash2, X, XCircle } from "lucide-react";
...
type GeneratedProduct = { name: string; description: string; category: string; price: number; aliases: string[]; tags: string[]; isFeatured: boolean; isNew: boolean };
...
export default function ProductsSettingsPage() {
  ...
  async function generateWithAi() {
    if (!aiPrompt.trim()) { setError("Describe the product you want AI to create."); return; }
    setGenerating(true); setError(null); setMessage(null);
    try {
      const generated = await apiFetch<GeneratedProduct>("/ai-actions/generate-product", {
        method: "POST",
        body: JSON.stringify({ prompt: aiPrompt.trim(), existingProducts: products.map((product) => ({ name: product.name, category: product.category })) })
      });
      setForm((current) => ({
        ...current,
        name: generated.name,
        description: generated.description,
        category: generated.category || "empanada",
        price: String(generated.price),
        aliases: generated.aliases.join(", "),
        tags: generated.tags.join(", "),
        isFeatured: generated.isFeatured,
        isNew: generated.isNew
      }));
      setAiPrompt("");
      setMessage("AI generated the product details. Review them, add an image if needed, then save the product.");
    } catch (err) { setError(err instanceof Error ? err.message : "Unable to generate product with AI."); }
  ...
  async function fileToDataUrl(file: File) {
    ...
    return new Promise<string>((resolve, reject) => {
      const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = () => reject(new Error(`Unable to read ${file.name}.`)); reader.readAsDataURL(file);
    });
  }

  async function addImages(event: ChangeEvent<HTMLInputElement>) {
    const files = Array.from(event.target.files ?? []); event.target.value = ""; if (!files.length) return; setError(null);
    try {
  ...
  function removeImage(index: number) {
    setForm((current) => { const imageUrls = current.imageUrls.filter((_, itemIndex) => itemIndex !== index); return { ...current, imageUrls, imageUrl: imageUrls[0] ?? "" }; });
  }
  ...
  async function remove(product: Product) {
    if (!window.confirm(`Delete ${product.name}? Existing orders are not changed.`)) return; setError(null); setMessage(null);
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-5 py-8 lg:px-8">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4"><div><div className="mb-2 text-sm text-foreground/45"><Link href="/settings" className="hover:text-foreground">Settings</Link> / Products</div><h1 className="text-3xl font-semibold tracking-tight">Products</h1><p className="mt-2 text-sm text-foreground/60">Manage the live product catalog used by ordering, pricing, and AI.</p></div><Link href="/settings/ai-instructions"><Button variant="secondary">AI Instructions</Button></Link></div>
      ...
      {!editingId ? <Card className="mb-5 border-accent/20 bg-accent/[0.03] p-5 lg:p-6"><div className="flex items-start gap-4"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-accent/20 bg-accent/10"><Sparkles className="h-5 w-5 text-accent" /></div><div className="min-w-0 flex-1"><h2 className="font-semibold">Generate with AI</h2><p className="mt-1 text-sm leading-6 text-foreground/55">Describe a new empanada or menu item. AI will draft the name, description, category, price, aliases, and tags without saving anything until you review it.</p><div className="mt-4 flex flex-col gap-3 sm:flex-row"><Input value={aiPrompt} onChange={(event) => setAiPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void generateWithAi(); } }} placeholder="e.g. A spicy chicken empanada with Filipino flavors" disabled={generating} /><Button type="button" onClick={() => void generateWithAi()} disabled={generating || !aiPrompt.trim()}><Sparkles className="h-4 w-4" />{generating ? "Generating…" : "Generate product"}</Button></div><p className="mt-2 text-xs text-foreground/40">The active AI model configured in Admin Settings will be used.</p></div></div></Card> : null}
      ...
      <Card className="mb-7 p-5 lg:p-6">
        ...
        <form onSubmit={submit} className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-2 text-sm font-medium lg:col-span-2"><span className="block">Product images</span><div className="flex flex-wrap gap-3">{form.imageUrls.map((src, index) => <div key={`${src.slice(0, 24)}-${index}`} className="group relative h-24 w-24 overflow-hidden rounded-lg border border-white/[0.1] bg-white/[0.03]"><img src={resolveImageUrl(src)} alt={`Product image ${index + 1}`} className="h-full w-full object-cover" /><button type="button" onClick={() => removeImage(index)} className="absolute right-1 top-1 rounded-full bg-black/70 p-1 text-white opacity-0 transition group-hover:opacity-100" aria-label={`Remove image ${index + 1}`}><X className="h-3.5 w-3.5" /></button>{index === 0 ? <span className="absolute bottom-0 left-0 right-0 bg-black/60 px-1 py-0.5 text-center text-[10px]">Primary</span> : null}</div>)}{form.imageUrls.length < MAX_IMAGES ? <label className="flex h-24 w-24 cursor-pointer flex-col items-center justify-center gap-1 rounded-lg border border-dashed border-white/[0.15] bg-white/[0.02] text-xs text-foreground/50 transition hover:bg-white/[0.05]"><ImagePlus className="h-5 w-5" /><span>Add image</span><input type="file" accept="image/*" multiple className="sr-only" onChange={addImages} /></label> : null}</div><p className="text-xs font-normal text-foreground/40">Upload up to {MAX_IMAGES} images, 2 MB each. The first image is used as the primary image.</p></div>
          ...
          <div className="flex flex-wrap items-center gap-5 pt-2 lg:col-span-4"><label className="flex items-center gap-3 text-sm font-medium"><input type="checkbox" checked={form.available} onChange={(e) => setForm({ ...form, available: e.target.checked })} /> Available for ordering</label><label className="flex items-center gap-3 text-sm font-medium"><input type="checkbox" checked={form.isFeatured} onChange={(e) => setForm({ ...form, isFeatured: e.target.checked })} /> Featured product</label><label className="flex items-center gap-3 text-sm font-medium"><input type="checkbox" checked={form.isNew} onChange={(e) => setForm({ ...form, isNew: e.target.checked })} /> New product</label></div>
      ...
      <Card className="overflow-hidden"><div className="border-b border-white/[0.07] px-5 py-4"><h2 className="font-semibold">Product catalog</h2></div>{loading ? <div className="p-6 text-sm text-foreground/50">Loading products…</div> : products.length === 0 ? <div className="p-8 text-center text-sm text-foreground/50">No products found.</div> : <div className="divide-y divide-white/[0.06]">{products.map((product) => <div key={product._id} className="flex flex-col gap-4 px-5 py-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex min-w-0 flex-1 items-start gap-3"><div className="h-14 w-14 shrink-0 overflow-hidden rounded-lg bg-white/[0.04]">{(product.imageUrls?.[0] ?? product.imageUrl) ? <img src={resolveImageUrl(product.imageUrls?.[0] ?? product.imageUrl)} alt={product.name} className="h-full w-full object-cover" /> : null}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-medium">{product.name}</span><Badge>{product.available ? "Available" : "Unavailable"}</Badge>{product.isFeatured ? <Badge>Featured</Badge> : null}{product.isNew ? <Badge>New</Badge> : null}{product.tags.map((tag) => <Badge key={tag}>{tag}</Badge>)}</div><p className="mt-1 text-sm text-foreground/50">₱{Number(product.price).toFixed(2)} · {product.category}{product.description ? ` · ${product.description}` : ""}</p>{product.aliases.length ? <p className="mt-1 text-xs text-foreground/40">AI aliases: {product.aliases.join(", ")}</p> : null}</div></div><div className="flex flex-wrap items-center gap-2"><Button variant="ghost" size="sm" onClick={() => void toggleAvailability(product)}>{product.available ? <><XCircle className="h-4 w-4" /> Mark unavailable</> : <><CheckCircle2 className="h-4 w-4" /> Mark available</>}</Button><Button variant="ghost" size="sm" onClick={() => edit(product)}><Pencil className="h-4 w-4" /> Edit</Button><Button variant="danger" size="sm" onClick={() => void remove(product)}><Trash2 className="h-4 w-4" /> Delete</Button></div></div>)}</div>}</Card>
    </main>
  );
}
