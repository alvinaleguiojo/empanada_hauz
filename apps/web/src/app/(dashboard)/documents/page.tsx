...
export default function DocumentsPage() {
  ...
  async function openFile(item: DocumentItem) {
    ...
    if (item.public) { window.open(`${API_URL}${item.url}`, "_blank", "noopener,noreferrer"); return; }
    try {
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-5 py-8 lg:px-8">
      ...
      <Card className="mb-5 p-3"><div className="flex flex-col gap-3 lg:flex-row lg:items-center"><div className="flex items-center gap-2 text-sm text-foreground/55"><span className="font-medium text-foreground">My Drive</span>{folderTrail.map((item, index) => <span key={item._id} className="flex items-center gap-2"><span>/</span><button type="button" onClick={() => goTrail(index)} className="hover:text-foreground">{item.name}</button></span>)}</div><div className="flex flex-1 items-center gap-2 lg:justify-end"><div className="relative min-w-0 flex-1 lg:max-w-md"><Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-foreground/35" /><input value={search} onChange={(event) => { setSearch(event.target.value); void load(folderId, event.target.value); }} placeholder="Search documents" className="w-full rounded-xl border border-white/[0.1] bg-background px-9 py-2.5 text-sm outline-none focus:border-accent/50" /></div><div className="flex rounded-xl border border-white/[0.1] p-1"><button type="button" onClick={() => setView("grid")} className={`rounded-lg p-2 ${view === "grid" ? "bg-white/[0.1]" : ""}`} aria-label="Grid view"><Grid2X2 size={16} /></button><button type="button" onClick={() => setView("list")} className={`rounded-lg p-2 ${view === "list" ? "bg-white/[0.1]" : ""}`} aria-label="List view"><List size={16} /></button></div></div></div></Card>
