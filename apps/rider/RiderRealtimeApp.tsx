...
async function getJobs(token: string) {
  const response = await fetch(`${API_URL}/rider/jobs`, { headers: { Authorization: `Bearer ${token}` } });
  if (!response.ok) throw new Error("Unable to load rider deliveries");
...
export default function RiderRealtimeApp() {
  ...
  const openMap = async () => {
    ...
    try {
      ...
      setMapData({
        riderLocation: lastLocation ? { latitude: lastLocation.latitude, longitude: lastLocation.longitude } : null,
        job: active
      });
      setMapOpen(true);
    } catch {
      ...
      <Modal visible={mapOpen} animationType="slide" onRequestClose={() => setMapOpen(false)}>
        <View style={styles.modal}>
          <RiderMapView
            ...
            dropoffAddress={mapData.job?.dropoffAddress}
...
const styles = StyleSheet.create({
  root: { flex: 1 },
  modal: { flex: 1, backgroundColor: "#101521" },
  mapButton: { position: "absolute", right: 18, bottom: 92, width: 58, height: 58, borderRadius: 29, backgroundColor: "#ef6637", alignItems: "center", justifyContent: "center", elevation: 8, shadowOpacity: 0.25, shadowRadius: 8, shadowOffset: { width: 0, height: 4 } },
  mapIcon: { color: "#fff", fontSize: 22, lineHeight: 23, fontWeight: "800" },
  mapLabel: { color: "#fff", fontSize: 10, fontWeight: "800", marginTop: 1 }
});
