...
type Props = { region: { latitude: number; longitude: number; latitudeDelta: number; longitudeDelta: number }; riderLocation: Coordinate | null; pickup: Coordinate | null; dropoff: Coordinate | null; destination: Coordinate | null; route?: Coordinate[] };

export const RiderMap = forwardRef<RiderMapHandle, Props>(function RiderMap({ region, riderLocation, pickup, dropoff, destination, route = [] }, ref) {
  const mapRef = useRef<MapView>(null);
  useImperativeHandle(ref, () => ({ recenter: () => mapRef.current?.animateToRegion(region, 400) }), [region]);
  const fallback = [riderLocation, destination].filter((point): point is Coordinate => !!point && Number.isFinite(point.latitude) && Number.isFinite(point.longitude));
  ...
  return <View style={styles.wrap}>
    <MapView ref={mapRef} style={StyleSheet.absoluteFill} provider={PROVIDER_GOOGLE} initialRegion={region} region={region}>
      {riderLocation ? <Marker coordinate={riderLocation} title="You"><View style={styles.riderMarker}><Text style={styles.motorcycle}>🏍️</Text></View></Marker> : null}
      ...
      {routePoints.length > 1 ? <Polyline coordinates={routePoints} strokeColor={colors.orange} strokeWidth={5} /> : null}
    </MapView>
    <Pressable style={styles.recenter} onPress={() => mapRef.current?.animateToRegion(region, 400)}><Text style={styles.recenterText}>◎</Text></Pressable>
  </View>;
});
...
const styles = StyleSheet.create({
  ...
  riderMarker: { minWidth: 40, height: 40, borderRadius: 20, backgroundColor: "#fff", alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: colors.orange, elevation: 4 },
