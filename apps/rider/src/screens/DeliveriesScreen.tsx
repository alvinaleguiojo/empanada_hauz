...
import { shortJobCode } from "../types";
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <ScrollView
        ...
        refreshControl={<RefreshControl refreshing={busy} onRefresh={() => void refresh()} tintColor={colors.orange} />}
      >
        ...
        {active.length ? active.map((job) => (
          <View key={job.id} style={styles.card}>
            <View style={styles.topRow}>
              ...
              <Text style={styles.fare}>₱{Number(job.finalFare ?? job.estimatedFare ?? 0).toFixed(2)}</Text>
            </View>
            <Text style={styles.address}>{job.pickupAddress}</Text>
            <Text style={styles.arrow}>↓</Text>
            <Text style={styles.dropoff}>{job.dropoffAddress}</Text>
            <Pressable style={styles.button} onPress={() => onOpenNavigation(job.id)}>
              <Text style={styles.buttonText}>Open navigation</Text>
            </Pressable>
          </View>
        ...
        {completed.length ? completed.map((job) => (
          <View key={job.id} style={styles.historyCard}>
            <View style={{ flex: 1 }}>
              <Text style={styles.customer}>{job.order?.customer?.name ?? "Customer"}</Text>
              <Text style={styles.historyMeta}>{shortJobCode(job.id)} · {job.status}</Text>
            </View>
            <Text style={styles.fare}>₱{Number(job.finalFare ?? job.estimatedFare ?? 0).toFixed(2)}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  section: { marginTop: spacing.xl, marginBottom: spacing.sm, fontSize: 12, fontWeight: "900", color: colors.muted, textTransform: "uppercase", letterSpacing: 1.5 },
  card: { backgroundColor: colors.surface, borderRadius: radius.lg, padding: spacing.md, marginBottom: spacing.sm, borderWidth: 1, borderColor: colors.line },
  topRow: { flexDirection: "row", gap: spacing.sm },
  status: { fontSize: 10, fontWeight: "900", color: colors.orangeDark, textTransform: "uppercase" },
  customer: { marginTop: 4, fontSize: 15, fontWeight: "900", color: colors.ink },
  fare: { fontSize: 15, fontWeight: "900", color: colors.ink },
  address: { marginTop: spacing.md, fontSize: 13, fontWeight: "700", color: colors.body },
  arrow: { marginVertical: 2, color: colors.muted },
  dropoff: { fontSize: 13, fontWeight: "700", color: colors.muted },
  button: { marginTop: spacing.md, backgroundColor: colors.ink, borderRadius: radius.md, paddingVertical: 12, alignItems: "center" },
  ...
  empty: { backgroundColor: colors.surface, borderRadius: radius.lg, padding: spacing.lg, alignItems: "center", borderWidth: 1, borderColor: colors.line },
