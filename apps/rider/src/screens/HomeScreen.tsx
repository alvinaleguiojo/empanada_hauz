...
export function HomeScreen({ session }: { session: RiderSession }) {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      >
        <Header online={rider.status === "online"} notificationCount={activeJobs.length} />

        <OnlineToggle
          online={rider.status === "online"}
          busy={busy}
          onToggle={() => setAvailability(rider.status === "online" ? "offline" : "online")}
        />
        ...
        {currentJob ? (
          <ActiveOrderCard job={currentJob} busy={busy} onPress={handleOrderPress} />
        ) : (
          <View style={styles.empty}>
            <Text style={styles.emptyEmoji}>🥟</Text>
            <Text style={styles.emptyTitle}>
              {rider.status === "online" ? "Waiting for an order" : "You're offline"}
            </Text>
            <Text style={styles.emptyText}>
              {rider.status === "online"
                ? "Keep the app open — we'll notify you the moment dispatch sends your next delivery."
                : "Flip the switch above when you're ready to start receiving deliveries."}
            </Text>
          </View>
        )}
        ...
        <View style={styles.recentCard}>
          {deliveredJobs.length ? (
            deliveredJobs
              .slice(0, 6)
              .map((job, index, arr) => <RecentOrderRow key={job.id} job={job} isLast={index === arr.length - 1} />)
          ) : (
            <Text style={styles.noRecent}>No deliveries completed yet today.</Text>
          )}
        </View>
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  emptyTitle: { fontSize: 15, fontWeight: "900", color: colors.ink, marginBottom: 4 },
  ...
  noRecent: { padding: spacing.lg, color: colors.muted, fontSize: 13, textAlign: "center" }
});
