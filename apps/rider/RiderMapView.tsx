...
type Props = { riderLocation?: Coordinate | null; pickup?: Coordinate | null; dropoff?: Coordinate | null; pickupAddress?: string; dropoffAddress?: string; onClose?: () => void };
...
function initialRegion(points: Coordinate[]): Region {
  ...
  const minLat = Math.min(...p.map(x => x.latitude));
  const maxLat = Math.max(...p.map(x => x.latitude));
  const minLng = Math.min(...p.map(x => x.longitude));
  const maxLng = Math.max(...p.map(x => x.longitude));
...
async function getRoute(origin: Coordinate, destination: Coordinate): Promise<RouteInfo> {
  ...
  if (!response.ok || data.status !== "OK" || !data.routes?.[0]) {
    if (data.status === "REQUEST_DENIED") throw new Error(data.error_message || "Directions API request was denied. Enable Directions API and check API key restrictions.");
    if (data.status === "ZERO_RESULTS") throw new Error("No driving route was found between these locations.");
...
export default function RiderMapView({ riderLocation, pickup, dropoff, pickupAddress, dropoffAddress, onClose }: Props) {
  ...
  // The active destination is pickup while the rider has not yet reached the store.
  // This intentionally does NOT route to drop-off through pickup, which was causing incorrect routes.
  const origin = valid(riderLocation) ? riderLocation : null;
  const destination = valid(pickup) ? pickup : valid(dropoff) ? dropoff : null;
  const mapPoints = useMemo(() => [riderLocation, pickup, dropoff].filter(valid), [riderLocation, pickup, dropoff]);

  const fitRoute = () => {
    if (mapPoints.length) map.current?.fitToCoordinates(mapPoints, { edgePadding: { top: 180, right: 50, bottom: 290, left: 50 }, animated: true });
  };

  useEffect(() => { requestAnimationFrame(fitRoute); }, [riderLocation?.latitude, riderLocation?.longitude, pickup?.latitude, pickup?.longitude, dropoff?.latitude, dropoff?.longitude]);
  ...
  useEffect(() => {
    ...
    getRoute(origin, destination)
      .then(result => { if (!cancelled) { setPath(result.points); setInfo(result); } })
      .catch(error => { if (!cancelled) { setPath([]); setInfo(null); setRouteError(error instanceof Error ? error.message : "Unable to calculate route."); } })
  ...
  useEffect(() => {
    if (following && valid(riderLocation)) map.current?.animateToRegion({ latitude: riderLocation.latitude, longitude: riderLocation.longitude, latitudeDelta: 0.012, longitudeDelta: 0.012 }, 450);
  }, [following, riderLocation?.latitude, riderLocation?.longitude]);

  return <View style={s.container}>
    <MapView ref={map} style={StyleSheet.absoluteFill} initialRegion={initialRegion(mapPoints)} showsUserLocation={valid(riderLocation)} showsMyLocationButton={false} onPanDrag={() => setFollowing(false)}>
    ...
    <View style={s.header}>
      <View style={s.menu}><Text style={s.menuText}>☰</Text></View>
    ...
    <View style={s.side}>
      {valid(riderLocation) ? <Pressable style={s.round} onPress={() => { setFollowing(true); if (valid(riderLocation)) map.current?.animateToRegion({ latitude: riderLocation.latitude, longitude: riderLocation.longitude, latitudeDelta: 0.012, longitudeDelta: 0.012 }, 450); }}><Text style={s.roundText}>⌖</Text></Pressable> : null}
      <Pressable style={s.round} onPress={fitRoute}><Text style={s.roundText}>◎</Text></Pressable>
    </View>

    <View style={s.current}>
      <View style={s.orderNum}><Text style={s.orderNumText}>1</Text></View>
      <View style={{ flex: 1 }}>
        <View style={s.row}><Text style={s.currentTitle}>Current Order</Text><View style={s.pickPill}><Text style={s.pickPillText}>{valid(pickup) ? "Pickup" : "Delivery"}</Text></View></View>
        <Text style={s.address} numberOfLines={1}>{valid(pickup) ? pickupAddress : dropoffAddress ?? "Location pending"}</Text>
        <Text style={s.sub}>{loading ? "Calculating road route…" : info ? `${info.distance.toFixed(1)} km • ${info.duration}` : routeError ?? "Route unavailable"}</Text>
      </View>
    </View>
    ...
    {onClose ? <Pressable onPress={onClose} style={s.close}><Text style={s.closeText}>×</Text></Pressable> : null}
  </View>;
}
