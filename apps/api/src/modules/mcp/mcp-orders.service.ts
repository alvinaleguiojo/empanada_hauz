...
export class McpOrdersService {
  ...
  private serializeOrder(order: OrderWithRelations, options?: { includeDeliveryFee?: boolean }) {
    const includeDeliveryFee = options?.includeDeliveryFee !== false;
    return { id: order.id, orderNumber: order.orderNumber, status: order.status, quantity: order.quantity, unitPrice: order.unitPrice, totalAmount: order.totalAmount, ...(includeDeliveryFee ? { deliveryFee: order.deliveryFee } : {}), deliveryMethod: order.deliveryMethod, paymentMethod: order.paymentMethod, location: order.location, address: order.address, preferredSchedule: order.preferredSchedule, scheduleReminderSentAt: order.scheduleReminderSentAt, items: order.items, notes: order.notes, createdAt: order.createdAt, updatedAt: order.updatedAt, customer: { id: order.customer.id, name: order.customer.name, phoneNumber: order.customer.phoneNumber, defaultAddress: order.customer.defaultAddress, notes: order.customer.notes, totalOrders: order.customer.totalOrders, totalSpent: order.customer.totalSpent, isVip: order.customer.isVip }, batch: order.batch ? { id: order.batch.id, name: order.batch.name, batchDate: order.batch.batchDate, isClosed: order.batch.isClosed } : null, delivery: order.delivery ? { id: order.delivery.id, areaGroup: order.delivery.areaGroup, status: order.delivery.status, scheduledAt: order.delivery.scheduledAt, eta: order.delivery.eta, trackingLink: order.delivery.trackingLink, riderName: order.delivery.riderName, riderPlate: order.delivery.riderPlate, bookingNotes: order.delivery.bookingNotes, copyPayload: order.delivery.copyPayload } : null, orderNotes: order.orderNotes.map((note) => ({ id: note.id, body: note.body, createdAt: note.createdAt })) };
  }
}
