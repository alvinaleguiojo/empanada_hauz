...
export class AiToolRegistryService {
  ...
  async configure(name: string, patch: AiActionConfigPatch, createdById: string) {
    ...
    return this.actionConfig.upsert(name, patch, createdById, { description: handler.definition.description });
  }
    ...
    return [
      {
        definition: {
          name: "list_products",
          description: "Read the live customer-facing product catalog. Use this whenever the customer wants to browse, view, see, know, compare, or ask about the shop's menu, products, available items, offerings, or their current prices. Interpret natural language semantically; do not require a specific keyword.",
          risk: "read",
      ...
      },
      {
        definition: {
          name: "get_product",
          description: "Read one specific available product by its name or alias when the customer asks about a particular item, flavor, or price.",
          risk: "read",
          inputSchema: {
            type: "object",
      ...
      },
      {
        definition: {
          ...
          description: "Read the current delivery pricing configuration when the customer asks about delivery charges, shipping cost, delivery rates, or how much delivery will cost.",
          risk: "read",
          inputSchema: { type: "object", properties: {}, additionalProperties: false }
        },
      ...
      },
      {
        definition: {
          ...
          description: "Read the customer's active order summary and details when they ask what they ordered or request their order details.",
          risk: "read",
          requiresCustomerContext: true,
          inputSchema: {
            type: "object",
            properties: { orderNumber: { type: "string" }, id: { type: "string" } },
            additionalProperties: false
          }
        },
      ...
      },
      {
        definition: {
          ...
          description: "Check the status of the customer's active order when they ask about tracking, progress, preparation, delivery, or current order status.",
          risk: "read",
          requiresCustomerContext: true,
          inputSchema: {
            type: "object",
      ...
      },
      {
        definition: {
          ...
          description: "Persist the customer's current pending purchase request. Use when the customer expresses intent to buy or provides product/quantity information for a new order, including additions or changes to an unfinished order. This never creates a real order and should be used even when the customer's wording is colloquial or indirect. When the customer provides only a follow-up detail, preserve the existing draft and update only that detail.",
          risk: "write",
          requiresCustomerContext: true,
          inputSchema: {
            type: "object",
      ...
      },
      {
        definition: {
          ...
          description: "Clear the pending order draft without affecting a real order.",
          risk: "write",
          requiresCustomerContext: true,
          inputSchema: { type: "object", properties: {}, additionalProperties: false }
        },
      ...
      },
      {
        definition: {
          name: "create_order",
          description: "Create a real order from the customer's complete validated pending order after explicit confirmation. Before execution, combine the persisted pending draft with any explicitly supplied fields. Never infer missing required checkout data; let application validation reject an incomplete order.",
          risk: "write",
          requiresCustomerContext: true,
          ...
          inputSchema: {
            ...
            required: ["customerName", "quantity", "deliveryMethod", "paymentMethod", "items", "confirmed"],
            additionalProperties: false
          }
        },
        execute: async (args, context) => {
      ...
      },
      {
        definition: {
          name: "update_order",
          description: "Update an existing active customer order when the customer asks to change its items, quantity, delivery, payment, address, or schedule.",
          risk: "write",
          requiresCustomerContext: true,
          inputSchema: {
            type: "object",
      ...
      },
      {
        definition: {
          name: "cancel_order",
          description: "Cancel an active customer order when the customer requests cancellation and explicitly confirms the cancellation.",
          risk: "write",
          requiresCustomerContext: true,
          requiresExplicitConfirmation: true,
          inputSchema: {
            type: "object",
            properties: { orderNumber: { type: "string" }, id: { type: "string" }, confirmed: { type: "boolean" } },
            required: ["confirmed"],
            additionalProperties: false
          }
        },
      ...
      },
      {
        definition: {
          name: "delete_order",
          description: "Delete an eligible active order when the customer requests deletion and explicitly confirms the deletion.",
          risk: "write",
          requiresCustomerContext: true,
          requiresExplicitConfirmation: true,
          inputSchema: {
