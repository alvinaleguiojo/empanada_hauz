...
export class AiOrderStatusContextService implements OnModuleInit {
  ...
  private reconstructActiveOrderState(context?: AiContext): OrderDetails | undefined {
    ...
    if (flavors.length) { reconstructed.flavors = flavors; reconstructed.quantity = flavors.reduce((sum, item) => sum + item.quantity, 0); reconstructed.totalAmount = flavors.reduce((sum, item) => sum + item.subtotal, 0); }
  ...
  private calculateMissingFields(details: OrderDetails) {
    ...
    const quantity = Number(details.quantity ?? 0); const flavorQuantity = (details.flavors ?? []).reduce((sum, item) => sum + Number(item.quantity || 0), 0);
  ...
  private async enrichOrderStatusContext(context?: AiContext, orderNumber?: string) {
    const recentMessages = [...(context?.recentMessages ?? [])]; const customerName = context?.customerName?.trim();
    try {
      let order: OrderStatusResult | undefined;
      if (orderNumber) { order = await this.mcpOrders.getOrder({ orderNumber }) as OrderStatusResult; if (customerName && order.customer?.name && order.customer.name.localeCompare(customerName, undefined, { sensitivity: "accent" }) !== 0) order = undefined; }
      else if (customerName) { const listResult = await this.mcpOrders.listOrders({ customerName, limit: 100 }); const latest = listResult.orders?.find((candidate) => !["completed", "cancelled"].includes(String(candidate.status))) as OrderStatusResult | undefined; if (latest?.id) order = await this.mcpOrders.getOrder({ id: latest.id }) as OrderStatusResult; }
      if (!order?.id && !order?.orderNumber) { const missingOrderIdMessage = "I couldn't find an order under your details. Please send your order ID so I can check the status."; recentMessages.push(`APPLICATION ORDER STATUS TOOL RESULT: No order was found${orderNumber ? ` for order ID \"${orderNumber}\"` : customerName ? ` for customer \"${customerName}\"` : ""}. Ask the customer for an order ID. Do not invent an order number or status.`); return { context: { ...context, recentMessages }, orderStatus: undefined, missingOrderIdMessage }; }
  ...
  private formatOrderSummary(order: OrderStatusResult) {
    const items = (order.items ?? []).filter((item) => item?.name && Number(item.quantity) > 0).map((item) => { const quantity = Number(item.quantity); const unitPrice = Number(item.price ?? PRICES[item.name.toLowerCase()] ?? 0); const subtotal = Number(item.subtotal ?? quantity * unitPrice); return { quantity, name: String(item.name), unitPrice, subtotal: Number.isFinite(subtotal) ? subtotal : quantity * unitPrice }; });
    ...
    lines.push("", "TOTAL", `Food: ₱${foodTotal.toFixed(0)}`, "", "DELIVERY"); lines.push(`Method: ${order.deliveryMethod === "maxim" ? "Maxim Delivery" : order.deliveryMethod}`);
    ...
    const schedule = this.formatSummarySchedule(order.preferredSchedule); if (schedule.date || schedule.time) { lines.push("", "SCHEDULE"); if (schedule.date) lines.push(`Delivery date: ${schedule.date}`); if (schedule.time) lines.push(`Preferred time: ${schedule.time}`); }
    lines.push("", "Thank you! 😊"); return lines.join("\n");
  }

  private formatSummarySchedule(value?: string | Date | null) {
    if (!value) return { date: undefined, time: undefined };
    const date = value instanceof Date ? value : new Date(String(value)); if (Number.isNaN(date.getTime())) return { date: String(value), time: undefined };
    ...
    const hasExplicitTime = !(date.getUTCHours() === 0 && date.getUTCMinutes() === 0 && !String(value).includes("T")); return { date: dateText, time: hasExplicitTime ? timeText : undefined };
  }

  private formatOrderStatusLine(order: OrderStatusResult) {
    const parts = [`Order ${order.orderNumber} status: ${order.status}.`]; if (order.delivery?.status) parts.push(`Delivery status: ${order.delivery.status}.`); if (order.delivery?.eta) parts.push(`ETA: ${this.formatDate(order.delivery.eta)}.`); if (order.delivery?.riderName) parts.push(`Rider: ${order.delivery.riderName}.`); if (order.delivery?.trackingLink) parts.push(`Track here: ${order.delivery.trackingLink}`); return parts.join(" ");
  }
  private formatDate(value?: string | Date | null) { if (!value) return "none"; return value instanceof Date ? value.toISOString() : String(value); }
  private extractOrderNumber(message: string) { const explicit = message.match(/\b(?:order\s*(?:id|number|no)\s*[:#-]?|order\s*#\s*)\s*([A-Za-z0-9-]{3,64})\b/i); return explicit?.[1]?.trim(); }
  private extractFollowUpOrderNumber(message: string, context?: AiContext) { const normalized = message.trim(); if (!/^[A-Za-z0-9-]{3,64}$/.test(normalized)) return undefined; if (/^(?:new|change|edit|update|modify|existing|old|same|yes|no)$/i.test(normalized)) return undefined; const recent = (context?.recentMessages ?? []).slice(-3).join(" "); return /order\s*(?:id|number|#)|send.*order/i.test(recent) ? normalized : undefined; }
  private isAvailabilityQuestion(message: string) { const lower = message.trim().toLowerCase(); return /\b(?:available|open|accepting\s+orders?|taking\s+orders?)\b/i.test(lower) || /\b(?:pwede|puwede)\b.*\b(?:order|mo\s+order|pa\s+order)\b/i.test(lower) || /\b(?:order|mo\s+order|mag[-\s]?order)\b.*\b(?:pa|pwede|puwede)\b/i.test(lower) || /\b(?:abli|bukas)\b.*\b(?:pa|karon|karun|order)\b/i.test(lower); }
  private isOrderStatusQuestion(message: string) { return /\b(order\s*status|status\s*(?:sa|of|for|my)?\s*order|where(?:'s| is)\s+my\s+order|asa(?:n)?\s+(?:na|ang)\s+(?:akong|my)\s+order|naa na ba\s+(?:ang|akong)\s+order|naabot na ba\s+akong\s+order|has my order been placed|did you place my order|have you placed my order|was my order placed|is my order placed)\b/i.test(message); }
  private isOrderSummaryQuestion(message: string) { return /\b(?:send|show|give|provide|what(?:'s| is))\b.*\b(?:summary|order summary|my order|order details)\b/i.test(message) || /\b(?:summary|order summary)\b/i.test(message); }
  private isExplicitOrderUpdate(message: string) { return /\b(?:change|update|modify|edit|replace|switch|correct|correction|add|remove)\b.*\b(?:my|the)\s+order\b/i.test(message) || /\b(?:my|the)\s+order\b.*\b(?:change|update|modify|replace|switch|add|remove)\b/i.test(message) || /\bchange\s+my\s+order\s+to\b/i.test(message) || /\bupdate\s+my\s+order\s+to\b/i.test(message); }
  private isConfirmationMessage(message: string) { const lower = message.trim().toLowerCase(); return /^(yes|yeah|yep|correct|confirmed|confirm|go ahead|proceed|okay proceed|okay do it|do it|place my order|place the order|place that order|order it|order that|that's correct|that is correct|everything is correct|all are correct)$/.test(lower); }
  private hasCompleteOrderState(details: OrderDetails) { if (!details) return false; const quantity = Number(details.quantity ?? 0); const flavorQuantity = (details.flavors ?? []).reduce((sum, item) => sum + Number(item.quantity || 0), 0); const deliveryComplete = details.deliveryMethod === "pickup" || (details.deliveryMethod === "maxim" && Boolean(details.address?.trim() && details.landmark?.trim() && details.contactNumber?.trim())); return details.flavors.length > 0 && quantity >= 10 && flavorQuantity === quantity && Boolean(details.deliveryMethod) && Boolean(details.paymentMethod) && deliveryComplete && details.missingFields.length === 0; }
  private toManilaIso(date?: string, time?: string) { if (!date || !time) return undefined; const match = time.match(/^(\d{1,2}):(\d{2})(?:\s*(AM|PM))?$/i); if (!match) return undefined; let hour = Number(match[1]); const minute = Number(match[2]); const meridiem = match[3]?.toUpperCase(); if (meridiem === "PM" && hour < 12) hour += 12; if (meridiem === "AM" && hour === 12) hour = 0; if (hour > 23 || minute > 59) return undefined; return new Date(`${date}T${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}:00+08:00`).toISOString(); }
  private looksLikeDateOnly(value: string) { return /^(?:\d{4}-\d{2}-\d{2}|(?:january|february|march|april|may|june|july|august|september|october|november|december)\s+\d{1,2}(?:,\s*\d{4})?|\d{1,2}[/-]\d{1,2}[/-]\d{2,4})$/i.test(value); }
}
