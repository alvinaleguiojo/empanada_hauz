...
export class AiAdminToolRegistryService {
  ...
  async getTools(): Promise<AiToolDefinition[]> {
    ...
    const tools: AiToolDefinition[] = [
      ...
      { name: "get_expenses", description: "Read expense records for a date range, including totals by category.", risk: "read", inputSchema: { type: "object", properties: { startDate: { type: "string", description: "Inclusive YYYY-MM-DD date; defaults to today." }, endDate: { type: "string", description: "Inclusive YYYY-MM-DD date; defaults to startDate." } }, additionalProperties: false } },
      { name: "get_expense_summary", description: "Read aggregated expense totals and category breakdowns for a date range.", risk: "read", inputSchema: { type: "object", properties: { startDate: { type: "string", description: "Inclusive date in YYYY-MM-DD format. If omitted, defaults to today." }, endDate: { type: "string", description: "Inclusive date in YYYY-MM-DD format. If omitted, defaults to startDate." } }, additionalProperties: false } },
      { name: "search_customers", description: "Find customers by name, phone, or Messenger identifier.", risk: "read", inputSchema: { type: "object", properties: { query: { type: "string" }, limit: { type: "number" } }, required: ["query"], additionalProperties: false } },
      { name: "search_orders", description: "Find orders by number, customer, phone, location, date, or status.", risk: "read", inputSchema: { type: "object", properties: { query: { type: "string" }, date: { type: "string" }, status: { type: "string" }, limit: { type: "number" } }, additionalProperties: false } },
