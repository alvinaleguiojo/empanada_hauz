...
export class AiAdminToolRegistryService {
  ...
  async getTools(): Promise<AiToolDefinition[]> {
    ...
    const genericReadTools = generic.filter((tool) => !["capture_order_draft", "clear_order_draft"].includes(tool.name));

    return [
      ...genericReadTools,
      {
        name: "get_order_metrics",
        description: "Read order metrics for today, this week, or this month.",
        risk: "read",
        inputSchema: { type: "object", properties: { range: { type: "string", enum: ["today", "week", "month"] } }, additionalProperties: false }
      },
      {
        name: "search_customers",
        description: "Find customer records by name, phone number, or Messenger identifier. Use this for customer identification before using a customer id.",
        risk: "read",
        inputSchema: { type: "object", properties: { query: { type: "string" }, limit: { type: "number" } }, required: ["query"], additionalProperties: false }
      },
      {
        name: "search_orders",
        description: "Find orders by order number, customer, phone number, location, date, or status.",
        risk: "read",
        inputSchema: { type: "object", properties: { query: { type: "string" }, date: { type: "string" }, status: { type: "string" }, limit: { type: "number" } }, additionalProperties: false }
      },
      {
        name: "create_customer",
        description: "Create a customer after search_customers finds no matching record and the administrator explicitly confirms the exact details.",
        risk: "write",
        requiresExplicitConfirmation: true,
        inputSchema: { type: "object", properties: { name: { type: "string" }, phoneNumber: { type: "string" }, defaultAddress: { type: "string" }, confirmed: { type: "boolean" } }, required: ["name", "confirmed"], additionalProperties: false }
      },
      {
        name: "create_order",
        description: "Create a real order after all required checkout information is known and the administrator explicitly confirms it.",
        risk: "write",
        requiresExplicitConfirmation: true,
        inputSchema: { type: "object", properties: { customerId: { type: "string" }, customerName: { type: "string" }, phoneNumber: { type: "string" }, quantity: { type: "number" }, deliveryMethod: { type: "string" }, paymentMethod: { type: "string" }, location: { type: "string" }, address: { type: "string" }, preferredSchedule: { type: "string" }, items: { type: "array" }, notes: { type: "string" }, confirmed: { type: "boolean" } }, required: ["customerName", "quantity", "deliveryMethod", "paymentMethod", "items", "confirmed"], additionalProperties: false }
      },
      {
        name: "update_order",
        description: "Update an existing order by a verified order id or order number. Requires administrator confirmation.",
        risk: "write",
        requiresExplicitConfirmation: true,
        inputSchema: { type: "object", properties: { customerId: { type: "string" }, orderNumber: { type: "string" }, id: { type: "string" }, status: { type: "string" }, notes: { type: "string" }, confirmed: { type: "boolean" } }, additionalProperties: false }
      },
      {
        name: "cancel_order",
        description: "Cancel an order only after confirmation and verification of its id or order number.",
        risk: "write",
        requiresExplicitConfirmation: true,
        inputSchema: { type: "object", properties: { customerId: { type: "string" }, orderNumber: { type: "string" }, id: { type: "string" }, confirmed: { type: "boolean" } }, additionalProperties: false }
      }
    ];
  }
  ...
  async execute(name: string, args: Record<string, unknown>, context: AdminToolContext) {
    ...
    if (["get_current_datetime", "get_order_summary", "get_my_orders", "check_order_status"].includes(name)) {
      return this.applicationTools.execute(name as AiApplicationToolName, args);
    }

    return this.registry.execute(name, args, {
      customerId: String(args.customerId ?? "admin"),
  ...
  async confirmPending(context: AdminToolContext, action: string, args: Record<string, unknown>) {
    ...
    if (!pending || pending.action !== action || pending.fingerprint !== expected) {
    ...
    switch (action) {
      case "create_customer":
        result = await this.customersService.createCustomer(args as { name?: string; phoneNumber?: string; defaultAddress?: string });
        break;
      case "update_order":
        result = await this.analyticsTools.updateOrder(args as { id?: string; orderNumber?: string; status?: string; notes?: string });
        break;
      case "create_order":
      case "cancel_order":
        result = await this.applicationTools.execute(action as AiApplicationToolName, { ...args, confirmed: true });
        break;
      default:
        result = await this.registry.execute(action, { ...args, confirmed: true }, { customerId: String(args.customerId ?? "admin"), conversationId: context.conversationId, channel: "admin" });
    }
  ...
  private async prepareWrite(name: string, args: Record<string, unknown>, context: AdminToolContext) {
    if (args.confirmed === true && isExplicitConfirmation(context.message)) {
      return this.confirmPendingDirect(name, stripConfirmation(args), context);
    }
