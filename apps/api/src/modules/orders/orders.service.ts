...
const EMPANADA_HAUZ_PICKUP = { address: "Empanada Hauz", latitude: 10.2760457, longitude: 123.8466921 };
...
export class OrdersService {
  ...
  async list({ date, search }: { date?: string; search?: string } = {}) {
    const filters = [date ? this.buildDateFilter(date) : undefined, search ? this.buildSearchFilter(search) : undefined].filter((filter): filter is Prisma.OrderWhereInput => Boolean(filter));
    const where = filters.length > 1 ? { AND: filters } : filters[0];
    return this.prisma.order.findMany({
      where, include: { customer: true, batch: true, delivery: true, orderNotes: { orderBy: { createdAt: "desc" } }, deliveryJobs: { where: { status: { notIn: ["cancelled"] } }, orderBy: { requestedAt: "desc" }, take: 1, include: { rider: { include: { user: { select: { id: true, name: true, email: true } } } } } } },
  ...
  private buildDateFilter(date: string) {
    ...
    const start = new Date(`${date}T00:00:00+08:00`); if (Number.isNaN(start.getTime())) return undefined;
  ...
  private buildSearchFilter(search: string): Prisma.OrderWhereInput | undefined {
    const query = search.trim(); if (!query) return undefined;
    return { OR: [
      { orderNumber: { contains: query, mode: "insensitive" } }, { location: { contains: query, mode: "insensitive" } }, { address: { contains: query, mode: "insensitive" } },
      { notes: { contains: query, mode: "insensitive" } }, { customer: { name: { contains: query, mode: "insensitive" } } }, { customer: { phoneNumber: { contains: query, mode: "insensitive" } } },
      { delivery: { areaGroup: { contains: query, mode: "insensitive" } } }, { delivery: { riderName: { contains: query, mode: "insensitive" } } }, { delivery: { riderPlate: { contains: query, mode: "insensitive" } } },
  ...
  async track(id: string) {
    const order = await this.prisma.order.findUnique({ where: { id }, include: {
      customer: true, delivery: true, orderNotes: { orderBy: { createdAt: "desc" }, take: 5 },
      deliveryJobs: { where: { status: { notIn: ["cancelled"] } }, orderBy: { requestedAt: "desc" }, take: 1, include: { rider: { include: { user: { select: { id: true, name: true, email: true } }, vehicles: { where: { isActive: true } }, locations: { orderBy: { createdAt: "desc" }, take: 1 } } } } }
    } });
    if (!order) throw new NotFoundException("Order not found");
    const job = order.deliveryJobs[0];
    return {
      id: order.id, orderNumber: order.orderNumber, status: order.status, quantity: order.quantity, unitPrice: order.unitPrice, totalAmount: order.totalAmount, deliveryFee: order.deliveryFee, discountAmount: order.discountAmount,
      deliveryMethod: order.deliveryMethod, paymentMethod: order.paymentMethod, location: order.location, address: order.address, preferredSchedule: order.preferredSchedule, items: order.items, notes: order.notes, adLabel: order.adLabel,
      createdAt: order.createdAt, updatedAt: order.updatedAt, customer: { name: order.customer.name, phoneNumber: order.customer.phoneNumber },
      delivery: order.delivery ? { status: order.delivery.status, areaGroup: order.delivery.areaGroup, scheduledAt: order.delivery.scheduledAt, eta: order.delivery.eta, trackingLink: order.delivery.trackingLink, riderName: order.delivery.riderName, riderPlate: order.delivery.riderPlate, bookingNotes: order.delivery.bookingNotes, copyPayload: order.delivery.copyPayload, updatedAt: order.delivery.updatedAt } : null,
      deliveryJob: job ? { id: job.id, status: job.status, riderId: job.riderId, pickupAddress: job.pickupAddress, pickupLatitude: job.pickupLatitude, pickupLongitude: job.pickupLongitude, dropoffAddress: job.dropoffAddress, dropoffLatitude: job.dropoffLatitude, dropoffLongitude: job.dropoffLongitude, estimatedDurationMinutes: job.estimatedDurationMinutes, estimatedArrivalAt: job.estimatedArrivalAt, updatedAt: job.updatedAt,
        rider: job.rider ? { name: job.rider.user.name, phoneNumber: job.rider.phoneNumber, plateNumber: job.rider.vehicles[0]?.plateNumber ?? null, vehicleType: job.rider.vehicles[0]?.type ?? null, location: job.rider.locations[0] ? { latitude: job.rider.locations[0].latitude, longitude: job.rider.locations[0].longitude, heading: job.rider.locations[0].heading, speed: job.rider.locations[0].speed, createdAt: job.rider.locations[0].createdAt } : null } : null
      } : null,
  ...
  private async resolveTrustedLineItems(items: OrderLineItemDto[] | undefined) {
    if (!items || items.length === 0) throw new BadRequestException("At least one order item is required.");
    return Promise.all(items.map(async (item) => {
      const quantity = Math.trunc(Number(item.quantity)); if (!Number.isFinite(quantity) || quantity < 1) throw new BadRequestException(`Invalid quantity for "${item.name}".`);
      const product = await this.productsService.resolveByName(item.name, { requireAvailable: true });
      if (!product) throw new BadRequestException(`Unknown or unavailable product: "${item.name}".`);
      return { name: product.name, quantity, price: Number(product.price), subtotal: quantity * Number(product.price) };
    }));
  }
  ...
  private parsePreferredSchedule(value?: string) {
    if (!value?.trim()) return undefined;
    const normalized = value.trim().toLowerCase();
    const now = new Date();
    const todayParts = new Intl.DateTimeFormat("en-CA", { timeZone: BUSINESS_TIME_ZONE, year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(now);
    const part = (type: string) => todayParts.find((item) => item.type === type)?.value;
    const today = `${part("year")}-${part("month")}-${part("day")}`;
    let datePart: string | undefined;
    if (normalized === "today") datePart = today;
    else if (normalized === "tomorrow") {
      const tomorrow = new Date(`${today}T12:00:00+08:00`); tomorrow.setUTCDate(tomorrow.getUTCDate() + 1);
      datePart = tomorrow.toISOString().slice(0, 10);
    } else if (/^\d{4}-\d{2}-\d{2}$/.test(normalized)) datePart = normalized;

    if (!datePart) {
      const parsed = new Date(value);
      if (Number.isNaN(parsed.getTime())) throw new BadRequestException(`Invalid preferred schedule: ${value}`);
      return parsed;
    }
  ...
  private async findReusableCustomer(dto: Pick<ManualOrderEntryDto, "customerName" | "phoneNumber" | "address">) {
    const phoneNumber = normalizePhoneNumber(dto.phoneNumber); const name = normalizeCustomerName(dto.customerName); const address = normalizeCustomerAddress(dto.address); if (!phoneNumber && !name) return null;
    const candidates = await this.prisma.customer.findMany({ where: { OR: [...(dto.phoneNumber ? [{ phoneNumber: dto.phoneNumber }] : []), { name: dto.customerName }] }, orderBy: { updatedAt: "desc" }, take: 25 });
    const phoneMatch = phoneNumber ? candidates.find((customer) => normalizePhoneNumber(customer.phoneNumber) === phoneNumber) : null; if (phoneMatch) return phoneMatch; if (!name || !name.includes(" ")) return null;
    const nameMatches = candidates.filter((customer) => normalizeCustomerName(customer.name) === name); if (address) { const addressMatch = nameMatches.find((customer) => normalizeCustomerAddress(customer.defaultAddress) === address); if (addressMatch) return addressMatch; } return nameMatches.length === 1 ? nameMatches[0] : null;
  }
  ...
  async updateStatus(id: string, status: OrderStatus) {
    ...
    const order = await this.prisma.order.update({ where: { id }, data: { status }, include: { customer: true, batch: true, delivery: true, orderNotes: { orderBy: { createdAt: "desc" } } } }); this.realtime.emit("orders.updated", order); return order;
  }
  ...
  async remove(id: string) { const existing = await this.prisma.order.findUnique({ where: { id } }); if (!existing) throw new NotFoundException("Order not found"); await this.prisma.order.delete({ where: { id } }); this.realtime.emit("orders.updated", { id, deleted: true }); return { ok: true }; }
  async addNote(id: string, body: string) { const order = await this.prisma.order.findUnique({ where: { id } }); if (!order) throw new NotFoundException("Order not found"); const note = await this.prisma.orderNote.create({ data: { orderId: id, body: body.trim() } }); this.realtime.emit("orders.updated", { id, note }); return note; }

  async exportToGoogleDrive(orderIds: string[]) {
    const ids = [...new Set(orderIds.filter((id) => typeof id === "string" && id.trim()).map((id) => id.trim()))]; if (!ids.length) throw new BadRequestException("No orders selected for export");
    const orders = await this.prisma.order.findMany({ where: { id: { in: ids } }, include: { customer: true, orderNotes: { orderBy: { createdAt: "desc" } } } }); if (!orders.length) throw new NotFoundException("No selected orders were found"); return this.googleDriveOrderExport.uploadOrders(orders);
  }
}

function normalizePhoneNumber(value?: string | null) { return (value ?? "").replace(/\D/g, "").replace(/^0(?=9)/, "63"); }
